...
static const cyaml_schema_field_t config_yaml_fields[] = {
    ...
    CYAML_FIELD_SEQUENCE("blacklist", CYAML_FLAG_POINTER, dns_proxy_config_yaml_t,
...
dns_proxy_blacklist_response_t dns_proxy_parse_response_type(const char* str) {
  if (strcmp(str, "not_found") == 0) return DNS_RESPONSE_NOT_FOUND;
  if (strcmp(str, "refused") == 0) return DNS_RESPONSE_REFUSED;
  if (strcmp(str, "resolve") == 0) return DNS_RESPONSE_RESOLVE;
